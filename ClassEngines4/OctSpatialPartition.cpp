...
OctNode::OctNode(glm::vec3 position_, float size_, OctNode* parent_) : octBounds(nullptr), 
parent(nullptr), children(), objectList(std::vector<GameObject*>())
{
...
OctSpatialPartition::OctSpatialPartition(float worldSize_) : root(nullptr), rayIntersectionList(std::vector<OctNode*>())
{
	...
	root->Octify(3);
	std::cout << "there are " << root->GetChildCount() << " children" << std::endl;
...
void OctSpatialPartition::AddGameObject(GameObject* gameObject_)
{
...
GameObject* OctSpatialPartition::GetCollision(Ray ray_)
{
	...
	for (auto cell : rayIntersectionList) {
		for (auto obj : cell->objectList) {
			BoundingBox j = obj->GetBoundingBox();
			if (ray_.IsColliding(&j)) {
				if (ray_.intersectDist < shortestDistance) {
					result = obj;
					shortestDistance = ray_.intersectDist;
				}
			}
		}
		if (result != nullptr) {
...
void OctSpatialPartition::AddObjectToCell(OctNode* cell_, GameObject* obj_) {
	if (obj_->GetBoundingBox().Intersects(cell_->GetBoundingBox())) {
		if (cell_->IsLeaf()) {
			...
			std::cout << "Added " << obj_->GetTag() << " to cell. Max: " << to_string(obj_->GetBoundingBox().maxVert) << std::endl;
		}
		else {
			for (int i = 0; i < CHILDREN_NUMBER; i++) {
